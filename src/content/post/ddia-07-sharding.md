...
划分键值数据，主流就两种思路。这两种思路的区别，恰好是开篇那条主线——聚（让相关数据放在一起）还是散（让数据分布均匀）——第一次正式登场。
...
| 偏向 | 聚：相关数据放在一起 | 散：负载分布均匀 |
| 典型系统 | HBase、Bigtable、CockroachDB、MongoDB（范围模式） | Cassandra、DynamoDB、Kafka（按 key 分区） |

![按键范围分片 vs 按哈希分片：聚与散的取舍](https://img.runningotter.com/2026/06/b9902f04b9cf7c34eddcbb2830333f0a.png)
...
## 6. 收尾：分片分散的是数据，需要你选的是「聚还是散」

DDIA 第 7 章看起来在讲「一份大数据怎么拆、拆完怎么定位」，但绕回开篇那条线，它真正讲的是一件事：分片用「分散」换来了线性扩展，但每一个分片决策，都是在「让相关数据放在一起」和「让负载分布均匀」之间做取舍——而且这个取舍会反复出现。

把这一章放到「聚 vs 散」这条线上，就很清楚了：

| 在哪里取舍 | 偏「聚」的一侧 | 偏「散」的一侧 | 各自的代价 |
|---|---|---|---|
| 主键怎么分 | 按键范围：范围扫描快 | 按哈希：负载分布匀 | 偏聚则顺序写产生热点；偏散则范围查询失效 |
